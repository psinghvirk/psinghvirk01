...
#if defined(NDEBUG)
# error "Btcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xddbdd94b8a4c96847cacacb99140e7950113d06a36b59f6ba5e9bbad5941e84d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Btcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Btcoin Signed Message:\n";
...
{
    // Btcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Btcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Btcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Btcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Btcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Btcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x89176532135259860e408fbe3dcf3e02df56d08a04db8ff6eab29bfb45c0be8c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e957985143a40c475535f9a485194f1b05304232b2e92fd07be82cd657393cee8d6d504bcd9eeceba139b65d912ff364a7afcc09413c98ab5d0497001a208404") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1565338182;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084542690;

        if (fTestNet)
        {
            block.nTime    = 1565338159;
            block.nNonce   = 385456869;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0ebb3418f668391330ae1511dbe1ea83b8f990a28e05b323fcbacb0f9ed1f397"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Btcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BtcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BtcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BtcoinMiner(CWallet *pwallet)
{
    printf("BtcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BtcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BtcoinMiner, pwallet));
}
